...
public class ReactionAttributesParser {
    ...
    private ReactionComponent createMetaboliteFromID(String reactionComponentParams) {
        ...
        int stoichiometry = metaboliteStoichiometry(productParts);
        if (stoichiometry == -1) {
            stoichiometry = 1;
        ...
        //TODO: Duplicated metabolites
        Metabolite metabolite = new Metabolite(metaboliteID, metaboliteID);
    ...
    private int metaboliteStoichiometry(String[] productParts) {
        try {
            return Integer.parseInt(productParts[0]);
